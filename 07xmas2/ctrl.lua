...
-- ПЕРВАЯ ВОЛНА (5 мин)
	if Every_5_minute == 1 then			-- 15 честов (5 нач. лвл, 5 ср. лвл и 5 выс. лвл)
		...
		for i = 1 , 5 , 1 do 
			...
			local Monster = CreateChaEx(1228, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster , 298050)
			Notice("Сундук "..i.." находится по координатам: "..x..","..y )
		end

		--Стандартный сундук Хаоса (ID 1229)
		for i = 1 , 5 , 1 do 
			local temp = math.random(1,150)
			local angle = math.random(1,360)
			local x, y = GetMonsterPosition(position[temp])
			local Monster = CreateChaEx(1229, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster, 298050)
		end

		--Сундук Хаоса выс. ур. (ID 1230)
		for i = 1 , 5 , 1 do 
			local temp = math.random(1,150)
			local angle = math.random(1,360)
			local x, y = GetMonsterPosition(position[temp])
			local Monster = CreateChaEx(1230, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster, 298050)
		end
		
		--Сундук Хаоса нач. ур. (ID 1195)
		for i = 1 , 1 , 1 do 
			local temp = math.random(1,150)
			local angle = math.random(1,360)
			local x, y = GetMonsterPosition(position[temp])
			local Monster = CreateChaEx(987, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster , 298050)
			Notice("Бд Дракон "..i.." находится по координатам: "..x..","..y )
		end
...
-- ПЯТАЯ ВОЛНА (25 мин)
	if Every_5_minute == 5 then			-- средние монстры
		...
		for i = 1 , 1 , 1 do 
			...
			local angle = math.random(1,360)
			local x, y = GetMonsterPosition(position[temp])
			local Monster = CreateChaEx(1195, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster , 298050)
			Notice("Кококо "..i.." находится по координатам: "..x..","..y )
		end
	end
...
-- СЕДЬМАЯ ВОЛНА (35 мин)
	...
	for i = 1 , 1 , 1 do 
			...
			local Monster = CreateChaEx(1195, x,  y, angle , 60, map_copy)
	            SetChaLifeTime(Monster , 298050)
			Notice("Курица "..i.." находится по координатам: "..x..","..y )
		end
		...
		if HowManyNoDead <= 10 then
			...
			if boss <= 70 then										-- 70% Олень Апокалипсиса
				if Now_Time == 5 or Now_Time == 13 or Now_Time == 21 then
					local BOSS  = CreateChaEx(1195, 145,  150,  145, 60, map_copy)		-- МиниСНВ
				else
			...
			elseif boss > 70 and boss <= 95 then					-- 25% Курица Судного Дня
				if Now_Time == 7 or Now_Time == 15 or Now_Time == 23 then
					local BOSS = CreateChaEx(1193, 145,  150,  145, 60, map_copy)		-- МиниСНВ
				else
...
function map_copy_before_close_07xmas2( map_copy )
	...
	if How_Many_Active > 0 then
		DealAllActivePlayerInMap(map_copy,"GiveSpecItem1")
	end
...
function can_open_entry_07xmas2( map ) 
	...
	-- -- Снежная война для игроков 0-175 уровней 	(5:00, 12:00, 21:00)
	-- -- Снежная война для игроков 175+ уровней 	(6:00, 13:00, 22:00)
	-- if Now_Time == 5 or Now_Time == 13 or Now_Time == 21 or Now_Time == 7 or Now_Time == 15 or Now_Time == 23  then
		-- return 1
	-- else
		-- return 0
	-- end
