...
function init_entry(map)
    ...
    SetMapEntryTime(map, "2007/1/19/22/0", "1/0/0", "0/0/30", "0/5/0")
end

function after_enter_hell3( role, map_copy )
	local cha_name = GetChaDefaultName( role )
	SystemNotice( role, "Таинственный голос: "..cha_name..", все. Это уже выше всего. Дальше ты не пройдешь!" )
end
...
function startspecialskill (monsterID)
	local Notice_all = {}
	Notice_all[10] = "Таинственный голос: Рисунок кровью! Меч фантома покарает смертных."
	Notice_all[11] = "Таинственный голос: Гори, Огненный демон!"
	Notice_all[12] = "Таинственный голос: Зловещий дух, помилуй смертных."
	Notice_all[13] = "Таинственный голос: Воскресни, душа Тирана!"
	Notice_all[14] = "Таинственный голос: Спустись с небес, о, великий Феникс!"
	Notice_all[15] = "Таинственный голос: Прибудь сюда, мой слуга."
	Notice_all[16] = "Таинственный голос: Пробудись, лорд Дракан!"
	Notice_all[17] = "Таинственный голос: Пробудись, адский Тидаль!"
...
if CheckMonsterDead ( AZRAEL[10] ) == 1 then
		...
		if CRY[10]==0 then
			local Notice_all = "Барон Фантом: О, нет... Запомните, я вернусь и мой меч вас покарает..."
		...
		if CRY[11]==0 then
			local Notice_all = "Огненный Демон: По-вашему лучше холод? Три тысячи лет я находился здесь и вот..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[11]=1
			BOSSXYSJ[980] = 12
			BOSSSJSJ[980] = 1	
		...
		if CRY[12]==0 then
			local Notice_all = "Злобное отродье: Моя сила очень велика! Нет! Смертные! Ахх..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[12]=1
			BOSSXYSJ[981] = 6
			BOSSSJSJ[981] = 6	
		...
		if CRY[13]==0 then
			local Notice_all = "Тайран: Что чувствует мое сердце...разочарование..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[13]=1
			BOSSXYSJ[982] = 4
			BOSSSJSJ[982] = 4	
		...
		if CRY[14]==0 then
			local Notice_all = "Феникс: Фениксы бессмертны. Вы самонадеяны...повелитель даровал всем нам вечную жизнь..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[14]=1
			BOSSXYSJ[983] = 12
			BOSSSJSJ[983] = 12	
		...
		if CRY[15]==0 then
			local Notice_all = "Отчаяние: О, нет...смертные, как вы могли победить моих братьев..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[15]=1
			BOSSXYSJ[984] = 16
			BOSSSJSJ[984] = 16	
		...
		if CRY[16]==0 then
			local Notice_all = "Дракан: Нет, смертный, ты ошибаешься...мы бессмертны..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[16]=1
			BOSSXYSJ[985] = 16
			BOSSSJSJ[985] = 16	
		...
		if CRY[17]==0 then
			local Notice_all = "Тидаль: Красный цвет...о, нет...это кровь...моя кровь..."
			MapCopyNotice ( map_copy , Notice_all )
			CRY[17]=1
			BOSSXYSJ[986] = 12
			BOSSSJSJ[986] = 12	
	...
	if CRY[10]==1 and CRY[11]==1 and CRY[12]==1 and CRY[13]==1 and CRY[14]==1 and CRY[15]==1 and CRY[16]==1 and CRY[17]==1  then
		if HELLCLOSETIME==300 then
			Notice("Таинственный голос: Я слежу за тобой. Наверное ты очень много о себе и своей силе возомнил... У тебя есть пару минут, чтобы укрыться до обрушение свода Абаддона.")
			HELLCLOSETIME = HELLCLOSETIME - 120
		end
		local closetime = HELLCLOSETIME
		for i = 1 , MAXNOTICE , 1 do
			if closetime ==NOTICETIME[i] then
				local Notice_all = "Внимание! До обрушения Абаддона 10-17 осталось "..closetime.." секунд. Всем людям рекомендуется покинуть это место."
				MapCopyNotice ( map_copy ,Notice_all )
			end
		end
