...
function SkillArea_Circle_Hx ( sklv )									--???????�"?l????????
	local side = 200 
	SetSkillRange ( 4 , side  )  
end 
...
function SkillSp_Hx ( sklv )										--????"Tiger Roar"??sp???L??
	local sp_reduce = 20 
	return sp_reduce 
end 
...
function SkillSp_Lyzy ( sklv )										--????"Inferno Wings"??sp???L??
	local sp_reduce = sklv * 1 
	return sp_reduce 
end 
...
function SkillSp_Shzg ( sklv )										--????"Holy Beam"??sp???L??
	local sp_reduce = sklv * 1 
	return sp_reduce 
end 
...
function SkillSp_Clcy ( sklv )										--????"Traversing"??sp???L??
	local sp_reduce = sklv * 1 
	return sp_reduce 
end 
...
function SkillPre_Hyps ( sklv )														--????"Inferno Blast"?l???j?????
end 
...
function SkillArea_Square_Hyps ( sklv )												--?????????????"?l????????
	local side = 250 
	local angle = 90 
...
function SkillSp_Hyps ( sklv )														--???????????????sp???L??
	local sp_reduce = sklv * 1 
	return sp_reduce 
end 

function Skill_Hyps_Begin ( role , sklv )												--????"Inferno Blast"?l?????Z??
	local sp = Sp(role) 
...
function Skill_Wk_End ( ATKER , DEFER , sklv ) 
	...
	if KS_ID == UnNormal_KS_ID then						----?�?????????
		...
		local Item_Use = GetChaItem ( ATKER , 1 , 9 )			----?????????j???
...
function Skill_Jsfb_End ( ATKER , DEFER , sklv )  
	...
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
		...
		EffectPanel_Add(DEFER, "STATE_JSFB", statelv, "s0102", EffectPanel_TimeConvertor(statetime))
end 
...
function SkillSp_Xzfy ( sklv ) --????"Seal of Elder"??sp???L??
...
EffectPanel_Add(DEFER, "STATE_XZFY�", statelv, "s0104", EffectPanel_TimeConvertor(statetime))
end
...
Check_Ys_Rem ( ATKER , DEFER )
EffectPanel_Add(DEFER, "STATE_AYZZ", statelv, "s0105", EffectPanel_TimeConvertor(statetime))
end


--??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????

function SkillSp_Yy ( sklv )										--????"Eagle's Eye"??sp???L??
	local sp_reduce = 10  
	return sp_reduce 
end 
...
function SkillSp_Hxqj ( sklv )										--????"Astro Strike"??sp???L??
...
function SkillSp_Zj ( sklv )										--????"Mighty Strike"??sp???L??
...
function SkillSp_Fnq ( sklv )										--????"Rousing"??sp???L??
	local sp_reduce = 25 
	return sp_reduce 
end 
...
----boss??u??????????
	...
	EffectPanel_Add(DEFER, "STATE_FNQ", statelv, "s0223", EffectPanel_TimeConvertor(statetime))
end 
...
function SkillSp_Kb ( sklv )										--????"Berserk"??sp???L??
	local sp_reduce = 15 
	return sp_reduce 
end 
...
function Skill_Kb_End ( ATKER , DEFER , sklv ) 
	...
	EffectPanel_Add(ATKER, "STATE_KB", statelv, "s0084", EffectPanel_TimeConvertor(statetime))
end 
...
function SkillSp_Swzq ( sklv )										--????"Primal Fist"??sp???L??
...
function SkillSp_Fh ( sklv )										--????"Revival"??sp???L??
   local sp_reduce = 50   
	return sp_reduce 
end 
...
--????----------------------------------------------
function SkillSp_BingX ( sklv )										--????"Skill Icy Fairy Enticement"??sp???L??
...
function SkillSp_Shpf ( sklv )										--????"Skill Harden"??sp???L??
   local sp_reduce = 40 + sklv * 4 
	return sp_reduce 
end 
...
function Skill_Shpf_End ( ATKER , DEFER , sklv ) 
	...
	EffectPanel_Add(DEFER, "STATE_SHPF", statelv, "s0225", EffectPanel_TimeConvertor(statetime))
end 
...
function Skill_Mlch_End ( ATKER , DEFER , sklv ) 
	...
	if j == 1 then
		...
		EffectPanel_Add(DEFER, "STATE_MLCH", statelv, "usil", EffectPanel_TimeConvertor(statetime))
	else
	end
...
function SkillSp_Gtyz ( sklv )										--????"Skill Will of Steel"??sp???L??
   local sp_reduce = 15  
	return sp_reduce 
end 
...
function SkillSp_Pj ( sklv )										--????"Skill Break Armor"??sp???L??
   local sp_reduce = 25  
	return sp_reduce 
end 
...
function SkillSp_Bc ( sklv )										--????"assassinate"??sp???L??
...
function Skill_Bc_Begin ( role , sklv )								--??????????????Z??
	local sp = Sp(role) 
...
function SkillSp_Guz ( sklv )										--????"Shadow Slash"??sp???L??
...
function Skill_Guz_Begin ( role , sklv )								--????????????????Z??
	local sp = Sp(role) 
...
function Skill_Guz_End ( ATKER , DEFER , sklv )									--??????????????�?????
	...
	if GetChaAIType( DEFER ) >= 21 then    --21?????BossAI???????Boss????boss?????????????????J???
		...
		Hp_Endure_Dmg ( DEFER , hpdmg ) 
		AddState( ATKER , DEFER , STATE_XY, statelv , statetime ) 
		EffectPanel_Add(DEFER, "STATE_XY", statelv, "s0086", EffectPanel_TimeConvertor(statetime))
	else 
		SetSus( DEFER , 0 ) 
	end 
	Check_Ys_Rem ( ATKER ,DEFER )						--?�????????
...
function Skill_Jf_End ( ATKER , DEFER , sklv ) 
	...
	----boss??u??????????
		...
		EffectPanel_Add(DEFER, "STATE_JF", statelv, "s0213", EffectPanel_TimeConvertor(statetime))
	else 
		SetSus( DEFER , 0 ) 
	end 
end 
...
function Skill_Lj_End ( ATKER , DEFER , sklv ) 
	...
	if IsPlayer( DEFER ) == 0 then -- ??? ?????? 
		dmg_fin = dmg_fin * 0.6 -- ????????????? ?????? ?? 5 ????? 
	elseif IsPlayer( DEFER ) == 1 then --??? ??????
		dmg_fin = dmg_fin * 0.2 -- ????????????? ?????? ?? 5 ????? 
	end  
	local statelv = sklv
	local statetime = 3
	Hp_Endure_Dmg( DEFER, dmg_fin ) 
	AddState ( ATKER , DEFER , STATE_XY , sklv , 3 )  
	EffectPanel_Add(DEFER, "STATE_XY", statelv, "s0214", EffectPanel_TimeConvertor(statetime))
end
...
function SkillSp_Ldc ( sklv )										--????"Hunter Strike"??sp???L??
	local sp_reduce = 10  
	return sp_reduce 
end 
...
function SkillSp_Hfs ( sklv )										--????"Recover"??sp???L??
	local sp_reduce = 20    
	return sp_reduce 
end 
...
elseif IsPlayer( DEFER ) == 1 then 
	local GetChaJob = GetChaAttr(DEFER, ATTR_JOB)
	if GetChaJob == 13 or GetChaJob == 14 or GetChaJob == 16 then
		dmg_fin = dmg_fin * 0.5 * 2
	else
		dmg_fin = dmg_fin * 0.5
	end
end 
local dmg_for_mob = dmg_fin * 2 -- ??? ??????
local dmg_for_BM = dmg_fin * 3 -- ??? ??????????
...
function Skill_Xlzh_End ( ATKER , DEFER , sklv )  
	...
	EffectPanel_Add(DEFER, "STATE_XLZH", statelv, "s0100", EffectPanel_TimeConvertor(statetime))
end 
...
function Skill_Fzlz_End ( ATKER , DEFER , sklv )  
	...
	EffectPanel_Add(DEFER, "STATE_FZLZ", statelv, "s0101", EffectPanel_TimeConvertor(statetime))
end 
...
function Skill_Tshd_End ( ATKER , DEFER , sklv )  
	...
	----boss??u??????????
		...
		EffectPanel_Add(DEFER, "STATE_TSHD", statelv, "s0103", EffectPanel_TimeConvertor(statetime))
end 
...
EffectPanel_Add(DEFER, "STATE_YS", statelv, "s0123", EffectPanel_TimeConvertor(statetime))	
end 
...
function Skill_Clxz_Use( role , sklv )										--???????????l???
	local statelv = sklv  
	local fleesb_dif = 3 * statelv 
	local fleesb =  FleeSb(role) + fleesb_dif  
	SetCharaAttr( fleesb , role , ATTR_STATEV_FLEE ) 
...
function SkillArea_Square_Lxjy ( sklv )										--????????????"?l????????
	local side = 400 
	SetSkillRange ( 4 , side )  	
end 

function SkillPre_Lxjy ( sklv )										--????"Meteor Shower"?l???j?????
end 
...
function SkillSp_Lxjy ( sklv )										--????"Meteor Shower"??sp???L??
...
function Skill_Lxjy_End ( ATKER , DEFER , sklv ) 
	...
	local statelv = sklv
	AddState ( ATKER , DEFER , STATE_XY , statelv , statetime )  
	EffectPanel_Add(DEFER, "STATE_XY", statelv, "s0112", EffectPanel_TimeConvertor(statetime))
end 

--???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????


function SkillArea_Circle_Lh ( sklv )										--????????"?l????????
...
function SkillPre_Lh ( sklv )										--????"Howl"?l???j?????
end 

function SkillSp_Lh ( sklv )										--????"Howl"??sp???L??
...
function SkillArea_Circle_LhCruz ( sklv )										--????????"?l????????
	local side = 100 + math.floor ( sklv * 20 ) 
	SetSkillRange ( 4 , side )   
end 
...
function SkillPre_LhCruz ( sklv )										--????"Howl"?l???j?????
end 

function SkillSp_LhCruz ( sklv )										--????"Howl"??sp???L??
	local sp_reduce = 20 - math.floor ( sklv * 0.5 ) 
	return sp_reduce 
end 
...
--??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
function SkillArea_Sector_Hxdj ( sklv )										--????????????"?l????????
...
function SkillPre_Hxdj ( sklv )										--????"Parry"?l???j?????
end 

function SkillSp_Hxdj ( sklv )										--????"Parry"??sp???L??
	local sp_reduce = 20 + sklv * 2  
	return sp_reduce 
end 
...
function SkillArea_Circle_Ymsl ( sklv )										--????????????"?l????????
...
function SkillPre_Ymsl ( sklv )										--????"Barbaric Crush"?l???j?????
end 

function SkillSp_Ymsl ( sklv )										--????"Barbaric Crush"??sp???L??
...
function SkillArea_Circle_Dzy ( sklv )									--???????????"?l????????
	local side = 300 + math.floor ( sklv * 30 ) 
	SetSkillRange ( 4 , side )   
end 

function SkillPre_Dzy ( sklv )										--????"Greater Heal"?l???j?????
end 
...
function SkillSp_Dzy ( sklv )										--????"Greater Heal"??sp???L??
...
function SkillArea_Circle_Dhfs ( sklv )									--???????????"?l????????
	local side = 300 + math.floor ( sklv * 30 ) 
	SetSkillRange ( 4 , side )   
end 
...
function SkillPre_Dhfs ( sklv )										--????"Greater Recover"?l???j?????
end 

function SkillSp_Dhfs ( sklv )										--????"Greater Recover"??sp???L??
...
elseif IsPlayer ( DEFER ) == 1 then
	local GetChaJob = GetChaAttr(DEFER,ATTR_JOB)
	if GetChaJob == 13 or GetChaJob == 14 or GetChaJob == 16 then
		dmg_fin = dmg_fin * 0.075 * 2.5
	else
		dmg_fin = dmg_fin * 0.075
	end
end 
local dmg_for_mob = dmg_fin * 2 --�� ����� �2, ��� �������
local dmg_for_BM = dmg_fin * 3 -- �� ����� �3, ��� �������
local dmg_for_wizards
if GetChaID( DEFER ) == 673 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 1493 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 967 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 959 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 807 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 933 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 1113 then
dmg_fin = dmg_for_mob
elseif GetChaID( DEFER ) == 1468 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1473 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1503 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1507 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1511 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1515 then
dmg_fin = dmg_for_BM
elseif GetChaID( DEFER ) == 1522 then
dmg_fin = dmg_for_BM
end
dmg_fin = ( dmg_fin + dmg_ElfSkill )
Hp_Endure_Dmg( DEFER, dmg_fin ) 
...
function SkillArea_Circle_Sf ( sklv )										--????"Tail Wind"?l????????
	local side = 1000  
...
function SkillPre_Rsd ( sklv )														--????"Magma Bullet"?l???j?????
end 

function SkillArea_Square_Rsd ( sklv )												--?????????"?l????????
	local side = 600
	SetSkillRange ( 4 , side )   
end 
...
function SkillSp_Rsd ( sklv )														--?????????????sp???L??
	local sp_reduce = sklv * 10 + 15   
	return sp_reduce 
end 

function Skill_Rsd_Begin ( role , sklv )												--????"Magma Bullet"?l?????Z??
	local sp = Sp(role) 
...
function SkillArea_Square_Zzzh ( sklv )										--????????????"?l????????
	local side = 300 
	SetSkillRange ( 3 , side ) 	
end 
...
function SkillArea_Square_Sdbz ( sklv )										--????????????"?l????????
	local side = 300 
	SetSkillRange ( 3 , side ) 	
end 
...
function SkillArea_Square_Synz ( sklv )										--????????????"?l????????
	local side = 500 
	SetSkillRange ( 3 , side ) 	
end 
...
function SkillArea_Square_Hfwq ( sklv )										--???????????"?l????????
...
function SkillArea_Square_Syzy ( sklv )										--???????????"?l????????
...
--???????�???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
function SkillArea_Circle_Xw ( sklv )										--????"Whirlpool"?l????????
	local side = 300  
	SetSkillRange ( 3 , side  )  
end
...
--????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
function SkillArea_Circle_Mw ( sklv )										--????"Fog"?l????????
...
function SkillArea_Circle_Lm ( sklv )										--????"Lightning Curtain"?l????????
	local side = 300  
	SetSkillRange ( 3 , side  )  
end
...
function SkillArea_State_Lm ( sklv )										
	...
	local statelv = sklv 
	SetRangeState ( STATE_LM , statelv  , statetime ) 									--?????????L????
end 
...
function SkillArea_Line_Ctd ( sklv )										--????"Penetrating Bullet"?l????????
...
function SkillPre_Ctd ( sklv )										--????"Penetrating Bullet"?l???j?????
end 

function SkillSp_Ctd ( sklv )										--????"Penetrating Bullet"??sp???L??
	local sp_reduce = 10 + sklv * 1 
	return sp_reduce 
end 
...
function SkillArea_Sector_Ssd ( sklv )										--????????"?l????????
...
function SkillPre_Ssd ( sklv )										--????"Dispersion Bullet"?l???j?????
end 

function SkillSp_Ssd ( sklv )										--????"Dispersion Bullet"??sp???L??
...
function SkillSp_Swcx ( sklv )										--????"Death Shriek"??sp???L??
   local sp_reduce = 50  
	return sp_reduce 
end 
...
function SkillSp_Syzm ( sklv )										--????"Headshot"??sp???L??
	local sp_reduce = sklv * 2 + 30  
	return sp_reduce 
end 
...
function SkillArea_Circle_Slzb ( sklv )										--????????"?l????????
...
function State_PKKBYS_Add ( role , statelv )
	local aspd_dif = 0
	if statelv == 1 then
		aspd_dif = 0.05 + statelv * 0.015
	elseif statelv == 2 then
		aspd_dif = 0.1 + statelv * 0.015
	elseif statelv == 3 then
		aspd_dif = 0.15 + statelv * 0.015
	elseif statelv == 4 then
		aspd_dif = 0.2 + statelv * 0.015
	elseif statelv == 5 then
		aspd_dif = 0.25 + statelv * 0.015
	end
	local aspdsa = ( AspdSa(role) + aspd_dif ) * ATTR_RADIX 
	SetCharaAttr( aspdsa , role , ATTR_STATEC_ASPD ) 
	ALLExAttrSet(role)
end
...
--???h??????--------------------------------------
function SkillSp_JSMF ( sklv )										--????"Zombie Sorcery Attack"??sp???L??
	local sp_reduce = 3 + sklv * 2   
	return sp_reduce 
end 

function SkillArea_Line_JSMF ( sklv )										--????"Corpse Attack Wave"?l????????
	local lenth = 800   
...
function SkillSp_HDSMF ( sklv )										--????"Fox Sorcery Attack"??sp???L??
	local sp_reduce = 30 + sklv * 2   
	return sp_reduce 
end 
...
--????h??????--------------------------------------
function SkillSp_HYMF ( sklv )										--????"Fox Sorcery Attack"??sp???L??
...
--???????--------------------------------------
--????????????????????????????z???
function SkillSp_HYMH ( sklv )										--????"Fascinate"??sp???L??
	local sp_reduce = 50 + sklv * 2   
	return sp_reduce 
end 
...
function SkillSp_FoxMagic ( sklv )										--????"Fox Sorcery Attack"??sp???L??
	local sp_reduce = 10 + sklv * 2   
	return sp_reduce 
end
...
function SkillSp_TZJSMagic ( sklv )										--????"Fox Sorcery Attack"??sp???L??
	local sp_reduce = 10 + sklv * 2   
	return sp_reduce 
end
...
function SkillSp_TZQZMagic ( sklv )										--????"Fox Sorcery Attack"??sp???L??
	local sp_reduce = 10 + sklv * 2   
	return sp_reduce 
end
...
function SkillArea_Line_JXJBFW ( sklv )										--????"Conch Ray"?l????????
	local lenth = 500
	local width = 200
	SetSkillRange ( 1 , lenth , width  )  
end
...
function SkillArea_Circle_JBXZSB ( sklv )										--???????????"?l????????
...
function SkillPre_JBXZSB ( sklv )										--????"Swirling Arm"?l???j?????
end 

function SkillSp_JBXZSB ( sklv )										--????"Swirling Arm"??sp???L??
	local sp_reduce = 20 - math.floor ( sklv * 0.5 ) 
	return sp_reduce 
end 
...
function SkillSp_CRXSF ( sklv )										--????"Fascinate"??sp???L??
...
function SkillSp_SXZZZ ( sklv )										--????"Fascinate"??sp???L??
	local sp_reduce = 50 + sklv * 2   
	return sp_reduce 
end 
...
function SkillSp_XBLBD ( sklv )										--????"Fascinate"??sp???L??
	local sp_reduce = 50 + sklv * 2   
	return sp_reduce 
end 
...
function SkillSp_BLGJ ( sklv )										--????"Icy Dragon Strike"??sp???L??
   local sp_reduce = 15   
	return sp_reduce 
end 

function SkillArea_Sector_BLGJ ( sklv )										--????"Icy Dragon Strike"?l????????
	local angle = 120  
	local radius = 800 
	SetSkillRange ( 2 , radius , angle  )  
end
...
function SkillArea_Circle_BHSD ( sklv )										--?????????????"?l????????
...
function SkillArea_Circle_wlxw ( sklv )										--????"Whirlpool"?l????????
	local side = 1000  
	SetSkillRange ( 3 , side  )  
end
...
--------------------------------------------------------------------??????L
function SkillArea_Circle_wllm ( sklv )										--????"Lightning Curtain"?l????????
	local side = 500  
	SetSkillRange ( 3 , side  )  
end
...
function Skill_JLFT_End ( ATKER , DEFER , sklv )
	...
	elseif Item_ID==681 then
		--SystemNotice ( ATKER , "Mordo Junior == "..Item_ID ) 
		--AddState( ATKER , ATKER , STATE_JLFT7, statelv , statetime ) --?????? 7
		AddState( ATKER , ATKER , STATE_JLFT3, statelv , statetime ) --?????? 7
...
function Skill_jlzb_Begin( role , sklv )	
	...
	local item_elf_hp = GetItemAttr(item_elf,ITEMATTR_URE)		--???????jhp
	local Num_JL = GetItemForgeParam ( item_elf , 1 )
...
function Skill_jlzb_End( ATKER , DEFER , sklv )
	...
	if ptnRoleType == 1 or ptnRoleType ==5 or ptnRoleType == 17 then   --???????l???
		if item_elf_type == 59 then
			local elf_str =  GetItemAttr( item_elf ,ITEMATTR_VAL_STR )		--????
			local elf_con =  GetItemAttr( item_elf ,ITEMATTR_VAL_CON )		--????
			local elf_agi =  GetItemAttr( item_elf ,ITEMATTR_VAL_AGI )		--??
...
function JLTX_usu(role)
	...
	local item_elf_type = GetItemType (item_elf )	
	if item_elf_type ~= 59 then				--?????u?�???
		SkillUnable(role)
		SystemNotice ( role , "You need to equip a pet Fairy" )  
		return 0
	end
	local item_elf_hp = GetItemAttr(item_elf,ITEMATTR_URE)		--???????jhp
	local item_elf_endure = item_elf_hp - 100			--??????�???????l????????
	if item_elf_hp<50 then
		SkillUnable(role) 
		SystemNotice ( role , "Fairy's HP must be more than 0 to use this skill" ) 
		return 
	end
	local item_count = CheckBagItem ( role , 855 )		--?????u?�??????
	if item_count <= 0 then    
		SkillUnable(role)
...
function SkillArea_Line_BkcjHX ( sklv )										--????"Conch Ray"?l????????
	local sklv = 8
...
-- ?????????? 1 ???. --
--------------------
 function SkillArea_Circle_FuShe1( sklv )									--???????�"?l????????	
	local  sklv = 1
...
function Skill_Ysc_End ( ATKER , DEFER , sklv ) 
	...
	AddState ( ATKER , DEFER , STATE_YS , statelv , statetime ) 
	EffectPanel_Add(DEFER, "STATE_YS", statelv, "s0123BK", EffectPanel_TimeConvertor(statetime))
end 
...
function Skill_thaitdrug_End( ATKER , DEFER , sklv )
	...
	local statetime = 10
	AddState( ATKER , DEFER , STATE_HPHMHF , statelv , statetime )
	SystemNotice ( ATKER , "y????1000HP,????10??" )
end
----------------------------------------?????????------------------------------------
--?????????????--
...
function Skill_dragonheart_Begin ( role , sklv ) 
	...
	local item_count = CheckBagItem ( atk_role , 1101 ) --h?????
	if item_count <= 0 then  
		SkillUnable(atk_role)   
		SystemNotice ( atk_role , "??????u????u???????????????" ) 
	end 
...
--	SystemNotice( role ,"ATTR_STATEV_STA?????h????")
	ALLExAttrSet(role)
end
...
--		SystemNotice(ATKER,"ATTR_STATEV_STR?????I�??")
	ALLExAttrSet(role)
end
function State_MWMXJ_Rem ( role , statelv )
	SetCharaAttr(0 ,role , ATTR_STATEV_STR)
--			SystemNotice(ATKER,"?????I�??")
	ALLExAttrSet(role)
end
...
--				SystemNotice(ATKER,"ATTR_STATEV_MXHP???L???")
	ALLExAttrSet(role)
end
function State_FFJGD_Rem ( role , statelv )
	SetCharaAttr(0 ,role , ATTR_STATEV_MXHP)
--					SystemNotice(ATKER,"???L???")
	ALLExAttrSet(role)
end
...
function Skill_zhanbuwawa_End ( ATKER , DEFER , sklv )
	...
	if defHp<1 then 
		SystemNotice(ATKER,defer_name.." ????????????????? "..defer_name.." ??a?")
		return
	end
	...
	if DBtype==6 then
		...
		SystemNotice(DEFER,"??????????????????????????????????????????????t?????")
		return
	end	
	if DBtype==7 then 
		AddState ( ATKER , DEFER , STATE_CUSI , 1 , 1 )
		SystemNotice(DEFER,"???????????????j?u??????�???????????????")
		return
	end	
end 
function Skill_WB_Begin ( atk )
	local Item_Use = GetChaItem ( atk , 1 , 9 )			----?????????j???
...
function GetExpRand ( role ,value,p1,p2)  ------value???????,p1?????l?????,p2?????????
	local exprandmin = {}
	exprandmin[1] = 1200 
	exprandmin[2] = 2400 
	exprandmin[3] = 4800 
...
function Skill_YJWB_Begin ( role )
	...
	if el == 1 then
		SystemNotice( role , "?????j?????????????????z???" )
		UseItemFailed ( role )
		return
	end
	local Item_Use = GetChaItem ( role , 1 , 9 )			----?????????j???
	if Item_Use == nil then
		SystemNotice( role , "????????z??????????a?")
		UseItemFailed ( role )
		return
	end
	local ItemID_Use = GetItemID ( Item_Use )			----?ID
...
function ItemUse_YJBT ( role )
	...
	elseif getrandom <= 400 then   
		charLv = Lv ( role )		
		exp_now = GetChaAttr (  role , ATTR_CEXP )----------??j???????
		if charLv < 10 then
			exp_new = exp_now + 100
	...
	elseif getrandom <= 410 then  
		...
		local message = cha_name.."????z????????????"..itemname  
		Notice ( message )
...
---------------??h??yu????OK------------------------------------------
function Item_zbdamo(role,caoshu,wz,xx)-----caoshu:???????,wz:????????u??,xx:???�???1??????o???????
------------------????????
local Item_damo = GetChaItem ( role , 2 , wz ) ----????????l??????
local Num = GetItemForgeParam ( Item_damo , 1 )--??????????
Num = TansferNum ( Num )
local Hole_Num = GetNum_Part1 ( Num )----------??o?j????
local zb_id = GetItemID ( Item_damo )		----????????ID
local zb_name = "??"..GetItemName ( zb_id ).."??"----????????????

-------------????h???�?
local Checkjl = CheckItem_CanJinglian ( Item_damo )
if Checkjl == 0 then--------?????h????0
	if zb_id~=0 then
		SystemNotice ( role , zb_name.."?????h!")
	end
SystemNotice ( role , "??????????j?"..(wz+1).."????????h???????")
return 0
end
------------------------?????h------------100%???
------------?????h????1
if Hole_Num <= 2 then
	...
	SystemNotice ( role , "??h???,"..zb_name.."????????"..addh.."???�??!")
	return 1
else
------------?????????h???
if xx~=1 then
SystemNotice ( role , "??h???,"..zb_name.."??????????????!")
end
return 0
end
...
---------------------------??????yu????OK----------------------
function Item_zbjl(role,zbxz,stwz,wqwz,Jlanlv,ykc,djjc,bkjc)---xzzb:?1?????0???????,stwz:?????????,wqwz?????a?Jlanlv???????j????,ykc??1?????j???�???
-----------????????
--------?????????????0???l???
if Jlanlv>9 then
Jlanlv=9
...
local Num = GetItemForgeParam ( Item_damo , 1 )  --?????????l??????
Num = TansferNum ( Num )----??????
local Hole_Num = GetNum_Part1 ( Num )-----???????

if  Hole_Num==0 then    
SystemNotice( role , ""..zb_name.."u?�????????�?????" )
return 0
end
---------??????????????l???
local ckzbxz=Check_StoneItemType ( Item_damo , Item_baoshi , Item_baoshi )------??????????????l???
...
if chk_stcaowu == Hole_Num and chk_stone==0 then   
	SystemNotice( role , ""..zb_name.."??u?�???????????"..jlanname )
--	UseItemFailed ( role )
	return 0
end
...
if  chk_stonemax == Hole_Num then
          if   chk_stone ~= 0    then
               SystemNotice( role , ""..zb_name.."???"..jlanname.."????????????u?�???????????????????")
		return 0
          end
          SystemNotice( role , ""..zb_name.."??j???u?�???????????????")
          return 0
end
...
-------??h????????u???
----??u???
---Item_zbdamo(role,caoshu,wz,xx)-----caoshu:???????,wz:????????u??,xx:???�???1??????o???????
...
-----------???????????u???
------Item_zbjl(role,zbxz,stwz,wqwz,Jlanlv,ykc,djjc,bkjc)---zbxz:?1?????0???????,stwz:?????????,wqwz?????a?Jlanlv???????j????,ykc:???????�????1?? djjc:????�????????1????,bkjc:?????�???z?????1
...
--?????? ?????????? 1200 ??????
function SkillPre_CJRsd ( sklv )														--????"Magma Bullet"?l???j?????
end 

function SkillArea_Square_CJRsd ( sklv )												--?????????"?l????????
	local side = 1800
	SetSkillRange ( 4 , side )   
end 
...
function SkillSp_CJRsd ( sklv )														--?????????????sp???L??
	local sp_reduce = sklv * 2 + 15   
	return sp_reduce 
end 

function Skill_CJRsd_Begin ( role , sklv )												--????"Magma Bullet"?l?????Z??
	local sp = Sp(role) 
