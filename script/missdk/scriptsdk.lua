...
function GetMap( mapid )
	if mapid > MapList.count or MapList.mapname[mapid] == nil then
		return "Неизвестное Название Карты "
	end
...
function ResetNpcInfo( npc, name )
	...
	InitNpcMission()
	NpcPointer = npc
	local str = "НПС ["..name.."] перезагружен!"
	PRINT( str )
	LG( "npcinit", str )
end
...
function GetNpcInfo( npc, name )
	...
	if NpcMissionList.count > 0 then
		LG( "mission", "Установить НПС новую метку для задания!" )
		SetNpcHasMission( npc, 1 )
	end
	NpcPointer = LUA_NULL
	local str = "Получить у НПС ["..name.."] информацию о сценарие данных, ID = "..NpcInfoList.count
	PRINT( str )
	LG( "npcinit", str )
end
...
function ModifyNpcInfo( npc, name, id )
	...
	if NpcMissionList.count > 0 then
		PRINT( "mission", "Установить метку задания для НПС!" )
		SetNpcHasMission( npc, 1 )
	else
		PRINT( "mission", "Невозможно поставить метку задания для НПС!" )
		SetNpcHasMission( npc, 0 )
	end
	NpcPointer = LUA_NULL
	local str = "Изменить у НПС ["..name.."] информацию о сценарие данных, ID = "..id
	PRINT( str )
	LG( "npcinit", str )
end
...
function NpcProc( character, npc, rpk, id )
	...
	if NpcInfoList[id] == nil then
		PRINT( "Невозможно получить функцию НПС! ID = ",  id )
		local npcname = GetCharName( npc )
		local str = npcname..": Привет. Ты меня ищешь? Извини, но я сейчас не могу с тобой поговорить!"
...
function NpcState( character, npcid, id )
   PRINT( "NpcState:character, npcid, NpcMissionList", character, npcid, id )
	if NpcInfoList[id] == nil or NpcInfoList[id].missionlist == nil then
		PRINT( "Невозможно получить функцию НПС! ID = ",  id )
		LG( "npc_error", "Невозможно получить функцию НПС! ID = ",  id )
		return LUA_FALSE
	end
...
function NpcInfoReload( name,  func )
	...
	if ret == LUA_FALSE or npc == nil or id == nil then
		print( "Ненайден функция ["..name.."] НПС!" )
		return
	end
	PRINT( "Доступ к функции НПС, pointer =, id = ", npc, id )
	...
	print( "Измение функции НПС ["..name.."] прошло успешно!" )
end
...
function ExchangeDataX( srcID, srcNum, tarID, tarNum )
	if srcID == nil or srcNum == nil or tarID == nil or tarNum == nil then
		LG( "npcinit_error", "ExchangeDataX: Fuction parameter errorЈЎsrcID, srcNum, tarID, tarNum", srcID, srcNum, tarID, tarNum )
		return
	end
...
function AddBerthPort( id, name )
	PRINT( "add harbor notice: ID["..id.."],ГыіЖЎ¶"..name.."]" )
	LG( "boat_init", "add harbor notice: ID["..id.."],ГыіЖЎ¶"..name.."]" )
...
function GetBerthData( id )
	if id == nil or BerthPortList[id] == nil or BerthPortList[id].name == nil then		
		return "Неизвестное Название Бухты "..id
	end
...
function TestDefPage()
	Talk( 1, "Бабушка: Привет, дитя мое!" )
	Text( 1, "Задание ", amp, 2 )
	Text( 1, "Торговля ", ct, 2 )

	Talk( 2, "Бабушка: Ничего, иди делай свои дела" )

	Talk( 3, "Бабушка: Вы можете мне помочь собрать листовые почки? Мне нужно 10 шт. Получить их можно от Таинственного куста, возле Серебряного Города." )
	Text( 3, "Хорошо, я понял.", ct )
	...
	Talk( 4, "Бабушка: Не забудьте принести лекарство врачу. Он должен быть рядом с баром в Серебряном городе. " )
	Text( 4, "Хорошо ", MultiTrigger, GetMultiTrigger(), 2 )
	InitFuncList()
	AddFuncList( CreditExchange, 0 ) 
	Test( 4, "Обменять золото ", MultiFunc, GetFuncList(), GetNumFunc() )
	InitFuncList()
	AddFuncList( CreditExchange, 1 ) 
	Test( 4, "Обменять предмет ", MultiFunc, GetFuncList(), GetNumFunc() )
	InitTrade()
	Weapon( 1721 )
	Weapon( 1722 )
	Weapon( 1723 )
...
function TestDefMission( id, name, misid, scriptid, npcid, areaid )
	...
	MisPrizeSelAll() 
	MisBeginTalk( "Спасибо за доставку моей посылки!" )
	MisBeginCondition( AlwaysFailure )
	MisReultTalk( "Спасибо за доставку моей посылки!" )
	MisHelpTalk( "Задание незавершенно! Пожалуйста, продолжайте." )
...
function TestRandMission()
   ...
   DefineRandMission( 10, "Случайное задание ", 8 )
...
function TestBorn()
	...
	TriggerAction( 1, HelpInfo, MIS_HELP_IMAGE, "РВКЦmap ЦёµјКЦІб.tga" )
