...
function after_create_entry(entry) 
    ...
    Notice("Îáúÿâëåíèå: Ïî êîðäèíàòàì ["..posx..","..posy.."] Ïîÿâèëñÿ ïîðòàë â Îñòðîâ Ìàæîðà.") --Í¨Öª±¾×é·þÎñÆ÷µÄËùÓÐÍæ¼Ò

    local EntryName = " Îñòðîâ Ìàæîðà "
    SetMapEntryEventName( entry, EntryName )
end

function after_destroy_entry_abandonedcity(entry)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
    Notice("Îáúÿâëåíèå: Ïî êîðäèíàòàì ["..posx..","..posy.."] Çàêðûëñÿ ïîðòàë â Îñòðîâ Ìàæîðà.") 

end

function after_player_login_abandonedcity(entry, player_name)
    map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) --È¡µØÍ¼Èë¿ÚµÄÎ»ÖÃÐÅÏ¢£¨µØÍ¼Ãû£¬×ø±ê£¬Ä¿±êµØÍ¼Ãû£©
    ChaNotice(player_name, "Îáúÿâëåíèå: Ïî êîðäèíàòàì ["..posx..","..posy.."] Ïîÿâèëñÿ ïîðòàë â Îñòðîâ Ìàæîðà.") --Í¨Öª±¾×é·þÎñÆ÷µÄËùÓÐÍæ¼Ò
...
function check_can_enter_abandonedcity(role, copy_mgr)
	...
	if Lv(Cha) >=30 and Lv(Cha) <=500 then
		...
		SystemNotice(role, "Òîëüêî ïåðñîíàæè îò 30 äî 45 ëâë ìîãóò âîéòè â Îñòðîâ Ìàæîðà!")
		return 0    
	end
end
...
function begin_enter_abandonedcity(role, copy_mgr) 
		...
		SystemNotice(role,"Âû âîøëè â Îñòðîâ Ìàæîðà ") 
