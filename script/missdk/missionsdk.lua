...
function NoRecord( character, record )
	if record == nil then
		SystemNotice( character, "NoRecord: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function HasState( character, state_id )
	if state_id == nil then
		SystemNotice( character, "HasState: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function IsChaType( character, type_id )
	if type_id == nil then
		SystemNotice( character, "IsChaType: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NoChaType( character, type_id )
	if type_id == nil then
		SystemNotice( character, "NoChaType: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	local ret = GetChaBody( character )
	if ret ~= type_id then
...
function Starteffect( character, npc, tp )	
	if tp == nil then
		PRINT( "Starteffect: Не удалось вызвать функцию!" )
		SystemNotice( character, "Starteffect: Не удалось вызвать функцию!" )
		return LUA_ERROR	
	end
...
function HasRecord( character, record )
	if record == nil then
		SystemNotice( character, "HasRecord: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	local ret = IsValidRecord( character, record )
	if ret == LUA_FALSE then
...
function NoFlag( character, id, flag )
	if id == nil or flag == nil then
		SystemNotice( character, "NoFlag: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end	
...
function NoConvoyNpc( character, misid )
	if misid == nil then
		SystemNotice( character, "NoConvoyNpc: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NoMission( character, id )
	if id == nil then
		SystemNotice( character, "NoMission: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NoRandMission( character, id )
	if id == nil then
		SystemNotice( character, "NoRandMission: Не удалось вызвать функцию " )
		return LUA_ERROR
	end
...
function NoMisssionFailure( character, id )
	if id == nil then
		SystemNotice( character, "NoMisssionFailure: Не удалось вызвать функцию " )
		return LUA_ERROR
	end
...
function IsMission( character, id )
	if id == nil then
		SystemNotice( character, "IsMission: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function AddNextFlag( character, id, startflag, count )
	if id == nil or startflag == nil or count == nil then
		SystemNotice( character, "AddNextFlag: Не удалось вызвать функцию!" )
	end
	local num = 0
	for i = 1, count, 1 do
...
function AddRMNextFlag( character, id, startflag, count )
	if id == nil or startflag == nil or count == nil then
		SystemNotice( character, "AddRMNextFlag: Не удалось вызвать функцию!" )
	end
	local num = 0
	for i = 1, count, 1 do
...
function HasFlag( character, id, flag )
	if id == nil or flag == nil then
		SystemNotice( character, "HasFlag: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	local ret = IsValidFlag( character, flag )
	if ret ~= LUA_TRUE then
...
function HasAllFlag( character, id, startflag, count )
	if id == nil or startflag == nil or count then
		SystemNotice( character, "HasAllFlag: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	local num = 0
	for i = 1, count, 1 do
...
function GetNumFlag( character, id, startflag, count )
	if id == nil or startflag == nil or count == nil then
		SystemNotice( character, "GetNumFlag: Не удалось вызвать функцию!" )
		return 0
	end
	local num = 0
	for i = 1, count, 1 do
...
function IsValue( id1, id2 )
	if id1 == nil or id2 == nil then
		PRINT( "IsValue: Не удалось вызвать функцию!" )
		LG( "mission_error", "IsValue: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	if id1 == id2 then
		return LUA_TRUE
...
function NoItem( character, itemid, count )
	if itemid == nil or count == nil then
		PRINT( "NoItem: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function BankNoItem( character, itemid, count )
	if itemid == nil or count == nil then
		PRINT( "BankNoItem: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function EquipNoItem( character, itemid, count )
	if itemid == nil or count == nil then
		PRINT( "EquipNoItem: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function CheckPoint (character,value)
	...
	if i~=1 then
		SystemNotice( character , "Убедитесь, что у вас есть Доказательство Смерти ")
		return LUA_FALSE
	end
	local Point_dead = GetChaItem2 ( character , 2 , value )
	local Point = GetItemAttr(Point_dead, ITEMATTR_VAL_STR)
	if Point<41 then
		SystemNotice( character , "Еще раз проверьте количество смертей ")
		return LUA_FALSE
	end
	return LUA_TRUE
end
...
function HaveNoItem (character,value)
	local i= CheckBagItem( character, value )
	if i~=0 then
		SystemNotice( character , "Убедитесь, что у вас нет Доказательства Смерти ")
		return LUA_FALSE
	end
	return LUA_TRUE
end
...
function AddChaItem1(character,npc,value)
	...
	if item_number >=1 then
		SystemNotice( character ,"У вас уже есть Письмо с Вызовом ")
		return LUA_FALSE 
	end
	local r1=0
	local r2=0
...
function AddChaItem2(character,npc,value)
	...
	if cha ==1 then
		SystemNotice( character ,"Вам необходимо сойти на берег, чтобы отметится в Письме с Вызовом ")
		return LUA_FALSE 
	end	
	local item1=GetItemP(character,0)
	local itemid1=GetItemID( item1 )
	if itemid1~=2911 then
		SystemNotice( character, "Разместите ваше Письмо с Вызовом в первой ячейке своего инвентаря." )
		return LUA_FALSE
	end
	local now_yes = os.date("%y")			
	local now_month= os.date("%m")			
	...
	if del_item==0  then
		SystemNotice ( character ,"Невозможно удалить Письмо с Вызовом ")
		return LUA_FALSE
	end
	...
	if used_time>28800 or used_time<0 then
		SystemNotice ( character ,"Молодец, ты завершил Вызов бытия! Но к сожалению, твое имя не будет вписано в книгу достижений Аскарона. Не отчаивайся, ты сможешь повторить состязание вновь.")
		return LUA_TRUE
	elseif used_time < 2400 and used_time>0 then
		Notice("Поздравляем игрока "..cha_name.." с успешным завершением Вызова бытия. Время игрока ("..used_time..") записано в Книгу рекордов Аскарона.")
	end
	if used_time < JINISI_TIME then
		JINISI_TIME=used_time
		LG( "JiNiSiJiLu_XinXi" , " Игрок "..cha_name.." прошел Вызов бытия за "..used_time.."." )
	end
	local r1=0
	local r2=0
	r1,r2 =MakeItem ( character , value  , 1 , 4 )
...
function AddChaItem3(character,npc,value)
	local item_number = CheckBagItem( character, value )			
	if item_number >=1 then
		SystemNotice( character ,"Вы уже имеете Карту Специальной Операции ")
		return LUA_FALSE 
	end
	local r1=0
	local r2=0
...
function AddChaItem4(character,npc,value)
	local i= CheckBagItem( character, value )
	if i~=1 then
		SystemNotice( character , "Убедитесь, что у вас есть Карта Специальной Операции ")
		return LUA_FALSE
	end
	local now_yes = os.date("%y")			
	local now_month= os.date("%m")			
	...
	if del_item==0  then
		SystemNotice ( character ,"Невозможно удалить Карту Специальной Операции ")
		return LUA_FALSE
	end
	local used_time=(now_scend - old_scend)+(now_miniute - old_miniute)*60+(now_hour - old_hour)*3600+(now_day - old_day)*86400+(JNSTime_Flag[now_month] - JNSTime_Flag[old_month] )*86400
	local cha_name = GetChaDefaultName ( character )
	if used_time>900 or used_time<0 then
		SystemNotice ( character ,"Вы не смогли уложится во времени. Попробуйте заного!")
		return LUA_FALSE
	elseif used_time <= 900 and used_time>0 then
		SystemNotice(character ,"Поздравляем, вы успели пройти специальное задание. Ваше время: "..used_time.."")
	end
	SynChaKitbag(character,13)
	return LUA_TRUE
end
...
function LessTime( character, value)
	...
	if HHS_Num ~= 1 then
		SystemNotice( character, "Убедитесь, что у вас есть Письмо Молодца ")
		return 0
	end
...
function JINNiuSS(character)
	local cha_name = GetChaDefaultName ( character )
	LG( "JINNiuSS" , "Игрок "..cha_name.." получил морской стимул " )
end

function JINNiuHD ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "JINNiuHD" , "Игрок "..cha_name.." получил пиратский стимул " )
end

function JINNiuCZ ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "JINNiuCZ" , "Игрок "..cha_name.." получил капитанский стимул " )
end

function ShuangZiSS (character,npc)
	local cha_name = GetChaDefaultName ( character )
	LG( "ShuangZiSS" , "Игрок "..cha_name.." морской стимул " )
end

function ShuangZiHD ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "ShuangZiHD" , "Игрок "..cha_name.." пиратский стимул " )
end

function ShuangZiCZ ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "ShuangZiCZ" , "Игрок "..cha_name.." получил капитанский стимул " )
end

function ZSSTART ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "ZSSTART" , "Игрок "..cha_name.." начал процесс реинкарнации " )
end

function ZSSTOP ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "ZSSTOP" , "Игрок "..cha_name.." закончил процесс реинкарнации " )
end

function ZSSTOP2 ( character,npc )
	local cha_name = GetChaDefaultName ( character )
	LG( "ZSSTOP2" , "Игрок "..cha_name.." закончил вторую реинкарнацию " )
end
...
function AddChaItem5(character,npc,value)
	local i= CheckBagItem( character, value )
	if i~=1 then
		SystemNotice( character , "Убедитесь, что у вас есть Карта специальной операции для дворца Тельца ")
		return LUA_FALSE
	end
	local now_yes = os.date("%y")			
	local now_month= os.date("%m")			
	...
	local del_item =TakeItem( character, 0,value, 1 )
	if del_item==0  then
		SystemNotice ( character ,"Невозможно удалить Карту специальной операции Тельца ")
		return LUA_FALSE
	end
	local used_time=(now_scend - old_scend)+(now_miniute - old_miniute)*60+(now_hour - old_hour)*3600+(now_day - old_day)*86400+(JNSTime_Flag[now_month] - JNSTime_Flag[old_month] )*86400
	local cha_name = GetChaDefaultName ( character )
	if used_time>420 or used_time<0 then
		SystemNotice ( character ,"Ваше время истекло. попробуйте еще раз.")
		return LUA_FALSE
	elseif used_time <= 420 and used_time>0 then
		SystemNotice(character ,"Поздравляем, вы завершили специальное задание. Ваше время: "..used_time..".")
	end
	SynChaKitbag(character,13)
	return LUA_TRUE
end
...
function CheckItem( character, itemid1, itemid2 )
	if itemid1 == nil or itemid2 == nil then
		PRINT( "CheckItem: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	...
	SystemNotice( character, "Разместите ваш Любовный амулет в соответствующую ячейку " )
		return LUA_FALSE	
	end
	return LUA_TRUE
end
...
function CheckBag( character, itemid, value1, value2)
	...
	if itemid== nil or value1== nil or value2== nil then
		PRINT( "CheckBag: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	...
	if itemid~=itemida or i~=value2 then
		SystemNotice( character, "Убедитесь, что с "..item_pos.." ячейки свободно хотя бы "..value2.." ячеек для "..item_name )
		return LUA_FALSE	
	end
	return LUA_TRUE
end
...
function CheckBagEmp( character,value)
	if value==nil then
		PRINT( "CheckBagEmp: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	...
	local cha = ChaIsBoat ( character ) 
	if cha ==1 then
		SystemNotice( character, "Чтобы бросить Вызов бытию, поговорите с Коди (2217,2911). Также освободите "..item_pos.." ячейку в вашем инвентаре. Всё игроки в море должны причалить к берегу.")
		return LUA_FALSE 
	end	
	if itemid~=0 then
		SystemNotice( character, "Чтобы бросить Вызов бытию, поговорите с Коди (2217,2911). Также освободите "..item_pos.." ячейку в вашем инвентаре. Всё игроки в море должны причалить к берегу.")
...
function CheckConvertProfession( character, newpf )
	if newpf == nil then
		PRINT( "CheckConvertProfession: Не удалось вызвать функцию!" )
		SystemNotice( character, "CheckConvertProfession: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function AcceptMission( character, npc, missionlist, tpindex )
	...
	if tp == WORLD_MISSION then
		...
		if missionlist == nil then
			PRINT( "AcceptMission: Не удалось вызвать функцию!missionlist = "..missionlist )
			SystemNotice( character, "AcceptMission: Не удалось вызвать функцию!missionlist = "..missionlist )
			LG( "mission_error", "AcceptMission: Не удалось вызвать функцию!missionlist = "..missionlist )
			return LUA_ERROR
		end
...
function CompleteMission( character, npc, missionlist, selitem, param )
	...
	if missionlist == nil then
		SystemNotice( character, "CompleteMission: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
	local npcid = GetCharID( npc )
	local ret, id, state, tp = GetMissionTempInfo( character, npcid )
...
function MissionProc( character, npc, rpk, missionlist )
	if missionlist == nil then
		SystemNotice( character, "MissionProc: Не удалось вызвать функцию!" )
		return LUA_FALSE
	end
	...
	elseif byCmd == MIS_BTNACCEPT then
		local ret = IsMissionFull( character )
		if ret == LUA_TRUE then
			return BickerNotice( character, "Список заданий переполнен. Задание не получено!" )
		end
...
function SendMissionLog( character, mission, id, name )
	...
	if id == nil or mission == nil then
		return SystemNotice( character, "Информации о задачах журнала не найдена,ID = "..id )
	end
...
function ObligeAcceptMission( character, id )
	...
	if ret == LUA_TRUE then
		SystemNotice( character, "Достигнут лимит заданий! Если вы хотите взять новое задание, то должны отменить или выполнить задания которые у вас имеются!" )
		return LUA_TRUE
	end
	local npcid = GetCharID( npc )
	...
	if ret ~= LUA_TRUE then
		SystemNotice( "ObligeMission:AcceptMission: Взять задание не удалось!" )
		LG( "mission_error", "ObligeMission:AcceptMission: Взять задание не удалось!" )
		return LUA_FALSE
	end
...
function MisPrizeProc( character, npc, mission, selitem, param )
	if mission == nil then
		SystemNotice( character, "MisPrizeProc: Не удалось вызвать функцию!" )
		return LUA_FALSE
	end
...
function MissionState( character, npcid, missionlist )
	PRINT( "MissionState" )
	if missionlist == nil then
		SystemNotice( character, "MissionState:Npc Сведения о задаче пусты!" )
		return LUA_FALSE
	end
	for i = 1, 32, 1 do
...
function RefreshMissionState( character, npc )
	if character == nil or npc == nil then
		PRINT( "RefreshMissionState: Не удалось вызвать функцию!" )
	end
...
function ActionsProc( character, actions, npc, page, misid, scriptid, param1, param2 )
	...
	for i = 1, num, 1 do
		...
		elseif actions[i].func == AddMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddMission Не удалось!" )
				SystemNotice( character, "ActionProc: Добавить задание не удалось!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == AddTrigger then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddTrigger Не удалось!" )
		...
		elseif actions[i].func == AddNpcTrigger then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddNpcTrigger Не удалось!" )
		...
		elseif actions[i].func == AddNextFlag then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddNextFlag Не удалось!" )
		...
		elseif actions[i].func == AddRMNextFlag then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddRMNextFlag Не удалось!" )
		...
		elseif actions[i].func == SetFlag then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:SetFlag Не удалось!" )
		...
		elseif actions[i].func == SetRecord then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:SetRecord Не удалось!" )
		...
		elseif actions[i].func == JINNiuSS then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:JINNiuSS Не удалось!" )
		...
		elseif actions[i].func == JINNiuHD then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:JINNiuHD Не удалось!" )
		...
		elseif actions[i].func == JINNiuCZ then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:JINNiuCZ Не удалось!" )
		...
		elseif actions[i].func == ShuangZiSS then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ShuangZiSS Не удалось!" )
		...
		elseif actions[i].func == ShuangZiHD then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ShuangZiHD Не удалось!" )
		...
		elseif actions[i].func == ShuangZiCZ then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ShuangZiCZ Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddReadingBook then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:AddReadingBook Не удалось!" )
		...
		elseif actions[i].func == AddExpPer then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddExpPer Не удалось!" )
		...
		elseif actions[i].func == ZSSTART then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ZSSTART Не удалось!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == AddExpNextLv1 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddExpNextLv1 Не удалось!" )
				SystemNotice( character, "ActionProc: AddExpNextLv1 Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddExpNextLv2 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddExpNextLv2 Не удалось!" )
				SystemNotice( character, "ActionProc:AddExpNextLv2 Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ZSSTOP then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ZSSTOP Не удалось!" )
		...
		elseif actions[i].func == ZSSTOP2 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ZSSTOP2 Не удалось!" )
				return LUA_FALSE
			end
...
--			actions[i].p2 = actions[i].p2  *GetExpState(character) /100
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddExp Не удалось!" )
				SystemNotice( character, "ActionProc: Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddSailExp then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddSailExp Не удалось!" )
				SystemNotice( character, "ActionProc: AddSailExp Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddLifeExp then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddLifeExp Не удалось!" )
				SystemNotice( character, "ActionProc: AddLifeExp Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddChaItem7 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:AddChaItem7 Не удалось!" )
				SystemNotice( character, "ActionsProc:AddChaItem7 Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddChaItem8 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:AddChaItem8 Не удалось!" )
				SystemNotice( character, "ActionsProc:AddChaItem8 Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == checkcytime then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:checkcytime Не удалось!" )
				SystemNotice( character, "ActionsProc:checkcytime Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == checksjtime then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:checksjtime Не удалось!" )
				SystemNotice( character, "ActionsProc:checksjtime Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddExpAndType then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddExpAndType Не удалось!" )
				SystemNotice( character, "ActionProc: AddExpAndType Добавить опыт не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddMoney then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: AddMoney Не удалось!param1 = %d", actions[i].p1 )
		...
		elseif actions[i].func == TakeMoney then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeMoney Не удалось!param1 = %d", actions[i].p1 )
				SystemNotice( character, "ActionProc: TakeMoney Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == teleport then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: Не удалось вызвать функцию \"teleport\"! Параметр 1 = %d", actions[i].p1 )
				SystemNotice( character, "ActionProc: teleport не удалось вызвать функцию!" )
				return LUA_FALSE
			end
		elseif actions[i].func == TakeCancelMissionMoney then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeCancelMissionMoney Не удалось!param1 = %d", actions[i].p1 )
				SystemNotice( character, "ActionProc: TakeCancelMissionMoney Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == GiveItem then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: GiveItem Не удалось!" )
				SystemNotice( character, "ActionProc: Добавить предмет персонажу не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == TakeItem then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeItem Не удалось!" )
		...
		elseif actions[i].func == TakeRandNpcItem then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeRandNpcItem Не удалось!" )
		...
		elseif actions[i].func == TakeAllRandItem then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeAllRandItem Не удалось!" )
		...
		elseif actions[i].func == ClearMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: ClearMission Не удалось!" )
		...
		elseif actions[i].func == ClearTrigger then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: ClearTrigger Не удалось!" )
		...
		elseif actions[i].func == DeleteTrigger then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: DeleteTrigger Не удалось!" )
		...
		elseif actions[i].func == ClearFlag then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: ClearFlag Не удалось!" )
		...
		elseif actions[i].func == ClearRecord then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ClearRecord Не удалось!" )
		...
		elseif actions[i].func == RefreshCompleteFlag then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:RefreshCompleteFlag Не удалось!" )
		...
		elseif actions[i].func == GiveMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:GiveMission Не удалось!" )
		...
		elseif actions[i].func == ObligeAcceptMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ObligeAcceptMission Не удалось!" )
		...
		elseif actions[i].func == ObligeCompleteMission then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc:ObligeCompleteMission Не удалось!" )
		...
		elseif actions[i].func == SummonNpc then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SummonNpc Не удалось!" )
				SystemNotice( character, "ActionsProc:SummonNpc Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ConvoyNpc then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:ConvoyNpc Не удалось!" )
				SystemNotice( character, "ActionsProc:ConvoyNpc Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ClearConvoyNpc then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:ClearConvoyNpc Не удалось!" )
				SystemNotice( character, "ActionsProc:ClearConvoyNpc Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == TakeXmasYB then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: TakeXmasYB Не удалось!" )
		...
		elseif actions[i].func == CpHuojiNum then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CpHuojiNum Не удалось!" )
				SystemNotice( character, "ActionsProc:CpHuojiNum Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == CpMiluNum then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CpMiluNum Не удалось!" )
				SystemNotice( character, "ActionsProc:CpMiluNum Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == CpXuerenNum then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CpXuerenNum Не удалось!" )
				SystemNotice( character, "ActionsProc:CpXuerenNum Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ClearAllConvoyNpc then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:ClearAllConvoyNpc Не удалось!" )
				SystemNotice( character, "ActionsProc:ClearAllConvoyNpc Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetMissionFailure then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetMissionFailure Не удалось!" )
				SystemNotice( character, "ActionsProc:SetMissionFailure Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetMissionComplete then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetMissionComplete Не удалось!" )
				SystemNotice( character, "ActionsProc:SetMissionComplete Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetTestTime then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: SetTestTime Не удалось!" )
		...
		elseif actions[i].func == RenewTestTime1 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: RenewTestTime1 Не удалось!" )
		...
		elseif actions[i].func == RenewTestTime2 then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionProc: RenewTestTime2 Не удалось!" )
		...
		elseif actions[i].func == FailureRandMissionCount then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:FailureRandMissionCount Не удалось!" )
				SystemNotice( character, "ActionsProc:FailureRandMissionCount Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetSpawnPos then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetSpawnPos Не удалось!" )
				SystemNotice( character, "ActionsProc:SetSpawnPos Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetProfession then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetProfession Не удалось!" )
				SystemNotice( character, "ActionsProc:SetProfession Не удалось!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == MultiTrigger then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:MultiTrigger Не удалось!" )
				SystemNotice( character, "ActionsProc:MultiTrigger Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SaveMissionData then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SaveMissionData Не удалось!" )
				SystemNotice( character, "ActionsProc:SaveMissionData Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetPkState then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetPkState Не удалось!" )
				SystemNotice( character, "ActionsProc:SetPkState Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == MoveCity then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:MoveCity Не удалось!" )
				SystemNotice( character, "ActionsProc:MoveCity Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == CreateGuild then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CreateGuild Не удалось!" )
				SystemNotice( character, "ActionsProc:CreateGuild Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ListAllGuild then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:ListAllGuild Не удалось!" )
				SystemNotice( character, "ActionsProc:ListAllGuild Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == RepairBoat then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:RepairBoat Не удалось!" )
				SystemNotice( character, "ActionsProc:RepairBoat Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SupplyBoat then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SupplyBoat Не удалось!" )
				SystemNotice( character, "ActionsProc:SupplyBoat Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == CreateBoat then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:CreateBoat Не удалось!" )
		...
		elseif actions[i].func == BoatLuanchOut then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:BoatLuanchOut Не удалось!" )
				SystemNotice( character, "ActionsProc:BoatLuanchOut Не удалось!" )
				return LUA_FALSE
			end			
		elseif actions[i].func == LuanchBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:LuanchBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:LuanchBerthList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == TradeBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:TradeBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:TradeBerthList Не удалось!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == RepairBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:RepairBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:RepairBerthList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SalvageBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SalvageBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:SalvageBerthList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SupplyBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SupplyBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:SupplyBerthList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == BoatLevelBerthList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:BoatLevelBerthList Не удалось!" )
				SystemNotice( character, "ActionsProc:BoatLevelBerthList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == BoatBerth then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:BoatBerth Не удалось!" )
				SystemNotice( character, "ActionsProc:BoatBerth Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == PackBagList then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:PackBagList Не удалось!" )
				SystemNotice( character, "ActionsProc:PackBagList Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == SetTradeItemLevel then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:SetTradeItemLevel Не удалось!" )
				SystemNotice( character, "ActionsProc:SetTradeItemLevel Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AdjustTradeCess then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:AdjustTradeCess Не удалось!" )
				SystemNotice( character, "ActionsProc:AdjustTradeCess Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == OpenBank then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:OpenBank Не удалось!" )
				SystemNotice( character, "ActionsProc:OpenBank Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == OpenHair then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:OpenHair Не удалось!" )
				SystemNotice( character, "ActionsProc:OpenHair Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == OpenRepair then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:OpenRepair Не удалось!" )
				SystemNotice( character, "ActionsProc:OpenRepair Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == RandFunction then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:RandFunction Не удалось!" )
				SystemNotice( character, "ActionsProc:RandFunction Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == AddCredit then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:AddCredit Не удалось!" )
				SystemNotice( character, "ActionsProc:AddCredit Не удалось!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == TakeOffer then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:TakeOffer Не удалось!" )
				SystemNotice( character, "ActionsProc:TakeOffer Не удалось!" )
				return LUA_FALSE
			end
		elseif actions[i].func == ValentinesRing then
			...
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:ValentinesRing Не удалось!" )
				SystemNotice( character, "ActionsProc:ValentinesRing Не удалось!" )
				return LUA_FALSE
			end
...
function NpcTriggerCheck( character, trigger )
	...
	if trigger == nil or trigger.conditions == nil then
		SystemNotice( character, "NpcTriggerCheck: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NpcTriggerResult( character, trigger )
	...
	if trigger == nil then
		SystemNotice( character, "NpcTriggerResult: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NpcCancelTrigger( character, trigger, misid, scriptid )
	...
	if trigger == nil or trigger.actions == nil then
		SystemNotice( character, "NpcCancelTrigger: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function NpcTrigger( character, npc, trigger, misid, scriptid )
	PRINT( "NpcTrigger" )
	if trigger == nil or trigger.actions == nil then
		SystemNotice( character, "NpcTrigger: Не удалось вызвать функцию!" )
		return LUA_ERROR
	end
...
function GetRandMissionPrize( character, misname, id, loopinfo, loopdata )
	...
	if loopinfo == nil or loopdata == nil or id == nil or misname == nil then
		...
		LG( "randmission_error", "GetRandMissionPrize: Не удалось вызвать функцию!prizeitem = nil and id = "..id )
...
function GetRandMissionExp( loopnum, miscount, exp )
	if loopnum == nil or miscount == nil or exp == nil then
		...
		LG( "randmission_error", "GetRandMissionExp: Не удалось вызвать функцию!" )
		return 0
	end
...
function GetRandMissionMoney( loopnum, miscount, money )
	if loopnum == nil or miscount == nil or money == nil then
		...
		LG( "randmission_error", "GetRandMissionMoney: Не удалось вызвать функцию!" )
		return 0
	end
...
function AddChaItem8(character)
	...
	if del_item==0  then
		SystemNotice ( character ,"Не удалось получить Экзаменационную бумагу ")
	end
	local used_time=(now_scend - old_scend)+(now_miniute - old_miniute)*60+(now_hour - old_hour)*3600+(now_day - old_day)*86400+(JNSTime_Flag[now_month] - JNSTime_Flag[old_month] )*86400
	local r1=0
	local r2=0
...
function checkcytime( character , npc )
	...
	if newch_xsz <= mch_xsz then
		SystemNotice ( character , "Поздравляем, Вы успешно сдали экзамен!")
		...
		SystemNotice( character , "Поздравляем, Вы успешно сдали экзамен и достигли максимального количества опыта!")
		AddExp ( character , npc, newexp_xsz , newexp_xsz )
	...
	if del_item == 0 then
		SystemNotice( character, "Не удалось получить Табель успеваемости ")
	end
	return LUA_TRUE
end
...
function checksjtime( character ,npc )
	...
	if del_item == 0 then
		SystemNotice( character, "Не удалось получить табель успеваемости по переводному экзамену ")
	end
	return LUA_TRUE
end
...
function XmasNotice ( role, value )
	...
	local message={}
	message[1]="Санта Клаус: Добро пожаловать в Рождественскую деревню, "..cha_name.."!"
	message[2]=""..cha_name..", эй, присоединяйся к нам. Будем пить, есть, гулять, танцевать. А думать будешь уже в следующем году!"
	message[3]=""..cha_name..", приходи на BOSS-арену и сразись в яростном увлекательном поединке."
