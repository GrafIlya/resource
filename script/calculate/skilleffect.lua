		...
		if dmgsa == 1 then
			...
			if Elf_Item ~= 0 then
				...
				if CheckElfSkill == 2 then
					...
					if ElfCrt == 1 then
						SystemNotice ( ATKER , "Фея активировала навык Берсерк. Бонус к атаке. " )
						SystemNotice ( DEFER , "Фея противника активировала навык берсерк. Бонус к атаке. " )
						dmgsa = 2
						SetSus( DEFER , sus )
					end
				end
...
function Skill_Ks_End ( ATKER , DEFER , sklv )
	SystemNotice ( ATKER , "Тук, тук..." ) 
	local defer_lv = GetChaAttr ( DEFER , ATTR_LV ) 
	if sklv < defer_lv then 
		SystemNotice ( ATKER , "Уровень умения слишком низкий. " ) 
		return 
	end 
	local hpdmg = 1 
	...
	if Tree_ID == UnNormal_Tree_ID then
		if Tree_hp <= 800 then
			hpdmg = 0
			SystemNotice( ATKER , "Ты не видишь ничего хорошего? Пройдет время, и ты почуствуешь пользу от работы " )
		end
	end
...
function Skill_Wk_End ( ATKER , DEFER , sklv )
	...
	if sklv < defer_lv then 
		SystemNotice ( role , "Уровень умения слишком низкий. " ) 
		return 
	end 
	local hpdmg = 1 
	...
	if KS_ID == UnNormal_KS_ID then						
		...
		if ItemID_Use ~= 3908 and ItemID_Use ~= 3108 then					
			SystemNotice( ATKER , "Можно использовать только Легированную кирку. " )
			return
		end
		...
		if Item_URE < 50 then						
			Take_Num = 0
			hpdmg = 0
			SystemNotice( ATKER , "Кирка повреждена. Не возможно использовать. " )
		end
		...
		if KS_hp <= 800 then
			hpdmg = 0
			SystemNotice( ATKER , "Метиорит исчерпан. отдохните! " )
		end
	end
	local hp = Hp(DEFER) - hpdmg 
	SetCharaAttr(hp , DEFER , ATTR_HP ) 
...
function Skill_By_End ( ATKER , DEFER , sklv )
	local defer_lv = Lv ( DEFER ) 
	if sklv < defer_lv then 
		SystemNotice ( ATKER , "Уровень умения слишком маленький. " ) 
		return 
	end 
	SystemNotice ( ATKER , "Идёт ловля рыбы..." ) 
	local hpdmg = 1 
	local hp = Hp(DEFER) - hpdmg 
	SetCharaAttr(hp , DEFER , ATTR_HP ) 
end 
...
function Skill_Dl_End ( ATKER , DEFER , sklv )
	...
	if sklv < defer_lv then 
		...
		SystemNotice ( ATKER , "Идёт процесс Траления..." ) 
	local hpdmg = 1 
	local hp = Hp(DEFER) - hpdmg 
	SetCharaAttr(hp , DEFER , ATTR_HP ) 
end 
...
function Skill_Xzfy_End ( ATKER , DEFER , sklv ) 
	...
	if Check_Heilong == 1 then
		local Percentage = Percentage_Random ( 0.7)
			if Percentage == 1 then
				...
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Урон умения увеличен. ")
			end
	end
	...
	if hp_defer >= 100000 then 
		...
		if a == 1 then
			...
			SetSus( DEFER , 0 ) 
			SystemNotice ( ATKER , "Не удалось использовать Печать Старейшены. ") 
			return
		end
	end 
	...
	if GetChaAIType( DEFER ) >= 21 then    
		if BOSSXZSJ[GetChaTypeID( DEFER )] == 0 then
			Notice ("Твои навыки на меня не действуют. ")
			return
		else
...
function Skill_Swzq_End ( ATKER , DEFER , sklv ) 
	...
	if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				  dmg =  dmg*3
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Атака увеличена. ")
			end
	end
	if IsPlayer ( DEFER ) == 1 and IsPlayer ( ATKER ) == 1 then
			...
			if Percentage == 1 then
				  Ran =  Ran*1.5
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Шанс урона увеличен.")
			end
	end
			...
			if dmg == Dmg_Min then
				SystemNotice ( ATKER , "Первобытная Ярость не попала по цели. " )
			end
	end
	Hp_Endure_Dmg ( DEFER , dmg )  
	...
	if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				...
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Время стана увеличено.")
			end
	end
	local hp_defer = Hp ( DEFER ) 
...
function Skill_Fh_Begin ( role , sklv ) 
	...
	if map_name_ATKER == "garner2" then
			SystemNotice ( role , "В данной локации нельзя использовать навык Воскрешение. " )
	SkillUnable(role)   
		end
	local item_count = CheckBagItem ( role , ITEM_RELIFE ) 
	if item_count <= 0 then  
		SkillUnable(role)   
		SystemNotice ( role , "У вас нету Оживляющего Клевера для использования навыка Воскрешение. " ) 
	end 
...
function Skill_BingX_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "Для использования необходим Ледяной кристалл. ")
		return
	end
...
function Skill_Mlch_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "Для активации требуется Магический Клевер. ")
		return
	end
	local b = ( sklv - 1 ) * 0.05
	...
	elseif a == 1 then
		j = 1
		SystemNotice ( ATKER , "Умение выполнено, без использования Магического Клевера. " )
	end
	if j == 1 then
		local statelv = sklv 
...
function Skill_Guz_End ( ATKER , DEFER , sklv )									
	...
	if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				 statetime = statetime * 2
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Эффект от умения увеличен. " )
			end
	end
	if  a == 1 then
...
function Skill_Lj_End ( ATKER , DEFER , sklv ) 
	...
	if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				  dmg =  dmg*1.5 
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Увеличена атака. ")
			end
	end
...
function Skill_Sj_End ( ATKER , DEFER , sklv ) 
	...
	if hp_defer >= 100000 then 
		...
		if a == 1 then
			...
			SetSus( DEFER , 0 )
			SystemNotice ( ATKER , "Вы полностью ослаблены. " ) 
			return
		end
	end 
	if GetChaTypeID( ATKER ) == 980 then
...
function Skill_Hzcr_End ( ATKER , DEFER , sklv ) 
	...
	local Check_Heilong  = CheckItem_Heilong ( role1)
		if Check_Heilong == 1 then
			...
			if Percentage == 1 then
				  statetime =  statetime*2
				SystemNotice ( ATKER , "Вы получили бонус от Снаряжения Черного Дракона. Продолжительность умения увеличено. ")
			end
	end
	local hp_defer = Mxhp ( DEFER ) 
	if hp_defer >= 1000000 then 
		...
		if a == 1 then
			...
			SetSus( DEFER , 0 )
			SystemNotice ( ATKER , "Не удалось использовать Переплетение Водрослей. " ) 
			return
		end
	end 
...
function Skill_PKXL_End ( ATKER , DEFER , sklv ) 
	...
	if i <= 0 then
		SystemNotice ( ATKER , "У вас нету древесины, ремонт не возможен. ")
		return
	end
	local j = DelBagItem(ATKER,4661,1)
	if j == 1 then
		SystemNotice ( ATKER , "Идёт ремонт сунда. " ) 
...
function Skill_Wudiyaoshui_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		...
		SystemNotice ( atk_role , "У вас нету Благословляющего зелья для использования навыка. " ) 
	end 
...
function Skill_Wudiyaoshui_End( ATKER , DEFER , sklv )
	...
	local message = cha_name.."Неуязвимость в течении 5 секунд "  
	Notice ( message )
end 
...
function Skill_jlzb_Begin( role , sklv )
	...
	if item_elf_type ~= 59 or Part1~=1 then
		SkillUnable(role) 
		SystemNotice ( role , "Текущий навык доступен только для фей второго поколения." ) 
		return 
	end
	if item_elf_hp<50 then
		SkillUnable(role)
		SystemNotice ( role, "Выносливость феи должна должна быть больше 0, чтобы использовать этот навык. ")
		return 
	end
...
function State_5MBS_Add(role , sklv)
	...
	SystemNotice (role,"Всевышний гневается на ваши действия. Приготовьтесь к наказанию. ")
end
...
function JLTX_usu(role)
	...
	if item_elf_type ~= 59 then				
		SkillUnable(role)
		SystemNotice ( role , "Вам необходимо иметь Фею для использования навыка. " )  
		return 0
	end
	local item_elf_hp = GetItemAttr(item_elf,ITEMATTR_URE)		
	...
	if item_count <= 0 then    
		SkillUnable(role)
		SystemNotice ( role , "Для использования навыка необходимо иметь Монету Феи. " ) 
		return 0
	end 
...
function Skill_Shoulei1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(role)   
		SystemNotice ( role , "Для использования навыка необходима Граната. " ) 
	end 
...
function Skill_ShanGD1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(role)   
		SystemNotice ( role , "Для использования навыка необходимо иметь Световую Бомбу. " ) 
	end 
...
function Skill_ShanGD1_End ( ATKER , DEFER , sklv ) 
	...
	local hp_defer = Mxhp ( DEFER ) 
	if hp_defer >= 1000000 then 
		SystemNotice ( ATKER , "Световая бомба потеряла эффект. " ) 
		return
	end 
...
function Skill_FuShe1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(role)   
		SystemNotice ( role , "Для использования навыка необходим Излучатель. " ) 
	end 
...
function Skill_YouL1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(role)   
		SystemNotice ( role , "Для использования навыка необходим Детектор Души. " ) 
	end 
...
function Skill_JiaSuQi1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(atk_role)   
		SystemNotice ( atk_role , "Для использования навыка необходим Корабельный ускоритель. " ) 
	end 
...
function Skill_PengSheQi1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(atk_role)   
		SystemNotice ( atk_role , "Для использования навыка необходим Распылитель Кораблей. " ) 
	end 
...
function Skill_PoJiaDan1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(atk_role)   
		SystemNotice ( atk_role , "Для использования навыка необходим Корабельный пробойник. " ) 
	end 
...
function Skill_PoRenDan1_Begin ( role , sklv ) 
	...
	if item_count <= 0 then  
		SkillUnable(atk_role)   
		SystemNotice ( atk_role , "Для использования навыка необходим Таран. " ) 
	end 
