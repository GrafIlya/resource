...
function Skill_Guz_End ( ATKER , DEFER , sklv )									--技能“鬼舞斩”的效果阶段
	...
	local statelv = sklv 
	local statetime = 2.5 + math.floor ( sklv * 0.4 ) 
	local map_name_ATKER = GetChaMapName ( ATKER )
	local map_name_DEFER = GetChaMapName ( DEFER )
	local agi_atker = Agi(ATKER)
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
...
function Skill_Jf_End ( ATKER , DEFER , sklv ) 
	...
	if MxHp_defer >= 1000000000 then
		SetSus( DEFER , 0 )
		return
	end

	if hp_defer >= 1000000000 then 
...
function Skill_Xzy_End ( ATKER , DEFER , sklv ) 
	local sta = Sta( ATKER ) 
	local exp = Exp( ATKER ) 
	local hpdmg = ( -1 ) * math.floor ( 90 + 90 * sklv + sta * 170 ) 
...
function SkillSp_Xlcz ( sklv )										--技能"Spiritual Bolt"的sp消耗公式
	local sp_reduce = 10 + sklv * 4   
	return sp_reduce 
end 

function SkillCooldown_Xlcz( sklv ) 
	local Cooldown = 4000 - sklv * 300
	return Cooldown
end
...
--	local atkdmg = Atk_Dmg ( ATKER , DEFER )
	...
	hpdmg = math.floor (( 100 + sta_atker * 8 ) * ( 1 + sklv * 0.75 ) * ( 1 +  lv_dif * 0.075 ))
		local map_name_ATKER = GetChaMapName ( ATKER )
	local map_name_DEFER = GetChaMapName ( DEFER )
	local agi_atker = Agi(ATKER)
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
		...
		hpdmg = math.floor (1.5 * dmg * ( 1 + sklv * 2.5 ) )
	--			Notice ( "statetime="..statetime)
			end
		end
...
function Skill_Bt_End ( ATKER , DEFER , sklv ) 
	...
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
		if map_name_ATKER == "garner2" or map_name_DEFER == "garner2" then
			if Can_Pk_Garner2 == 0 then
				dmg = math.floor ( dex_atker*2 + 30 * sklv +hp * ( 0.05 +0.05 * sklv) ) 
	--			Notice ( "dmg="..dmg)
			end
		end
	if dmg > 25000 then
...
function Skill_Bkcj_End ( ATKER , DEFER , sklv ) 
	...
	local dmg = math.floor( 2500 + sklv*100 +sta_atk*60 ) 
	...
	local Can_Pk_Garner2 = Is_NormalMonster (DEFER)
		if map_name_ATKER == "garner2" or map_name_DEFER == "garner2" then
			...
			dmg = math.floor(MAGIC_Atk_Dmg(ATKER,DEFER)+sklv*8)
...
function State_Synz_Add ( role , statelv ) 
	...
	local mspdsa_dif = (-9) * ( 9.99 + 9.99 * statelv ) * 9
	local mspdsa = math.floor (  (MspdSa(role) + mspdsa_dif ) * ATTR_RADIX ) 
...
----黑龙炎球
 ...
 function Skill_BlackYq_End ( ATKER , DEFER , sklv )
	local Sta_role = Sta ( DEFER ) 
	hpdmg = 1500000 +  math.max ( 50 , ( 150 - Sta_role ) )  * 20
	Hp_Endure_Dmg ( DEFER, hpdmg )
end 
...
function Skill_yghf_End ( ATKER , DEFER , sklv )
	...
	if hp_now < 100000000 then
	local	hp_rec = 100000000 
	end
		...
		if Part1 == 1 then 
			...
			if Item_ID ==232 then 
				...
				SetCharaAttr(star, role, ATTR_STATEV_STR)
                        ...
                        if Item_ID == 231 then 
                                ...
                                SetCharaAttr(star ,role , ATTR_STATEV_LUK)
                                local star = GetChaAttr( role, ATTR_STATEV_PDEF ) + 5
                                SetCharaAttr(star ,role , ATTR_STATEV_PDEF)
			end

			if Item_ID ==68 then 
				local star = GetChaAttr( role , ATTR_BCON )*5
				local star = lv_JL*2
			...
			if Item_ID ==236 then 
				...
				SetCharaAttr(star ,role , ATTR_STATEV_AGI)
                        ...
                        if Item_ID == 681 or Item_ID == 48 or Item_ID == 68 then 
			...
			if Item_ID == 681 or ItemID == 68 or ItemID == 48 then 
				if str~=nil and str~=0 then
					local star = str
					SetCharaAttr(star ,role , ATTR_STATEV_STR)
				end
...
function Skill_ShouWangS_End ( ATKER , DEFER , sklv ) 
        ...
        local dmg = 500  + ( sklv * 30 )
	Hp_Endure_Dmg ( DEFER , dmg ) 
	AddState ( ATKER, DEFER , STATE_DZFS , 10 , 15 ) -- makes em slow
...
function SkillArea_Circle_EmoYuYan ( sklv )
    --local sklv=1
	local side = 50 + math.floor ( sklv * 1 ) 
	SetSkillRange ( 4 , side )  
end
...
function State_EmoYuYan_Add ( role , statelv )
	local hpdmg = 10 + statelv * 1		-- (20 + 15 per skill lv) poison damage per sec for 15 sec
	Hp_Endure_Dmg ( role , hpdmg )
	ALLExAttrSet( role ) 
end 
